...
class analysis_data(object):
    ...
    def data_ano(self,msg):
        ...
        """if msgHead.get("msgID")==1794:
            msgBody=self.body_0702(databody)
        else:
            msgBody=''
           """    #0702 msgID
    ...
    """def up_0702(self,body_0702):
        databody=body.replace(" ","")
        car_status=jctool.to_int(databody[0:1])   #01插卡，02拔卡
        car_time=jctool.to_int(databody[1:7])  #系统时间
        read_car=jctool.to_int(databody[8])    #读卡结果，默认成功
        drivernamelength=jctool.to_int(databody[9])
